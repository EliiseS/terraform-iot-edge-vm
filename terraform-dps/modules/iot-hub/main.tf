...
resource "azurerm_iothub_shared_access_policy" "sense" {
  name                = "iot_hub_sas"
  resource_group_name = var.resource_group_name
  iothub_name         = azurerm_iothub.sense.name
...
resource "shell_script" "create_enrollment_group" {
  ...
  environment = {
    ...
    DPS_NAME                = azurerm_iothub_dps.iot_dps.name
    RESOURCE_GROUP_LOCATION = var.location
    IOT_HUB_NAME_ARR        = azurerm_iothub.sense.name # This could contain an array of IoT Hub names
    TERM                    = "xterm"
  }
...
resource "azurerm_iothub_shared_access_policy" "iot_hub_dps_shared_access_policy" {
  ...
  depends_on = [azurerm_iothub.sense]
}
