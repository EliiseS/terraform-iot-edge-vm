...
CERTIFICATE_DIR="${SCRIPT_DIR}/gen"
...
DEFAULT_VALIDITY_DAYS=${DEFAULT_VALIDITY_DAYS:=3600}
...
###############################################################################
# Generates a root certificate for CA certs only.
###############################################################################
...
if [ "${1}" == "create_root_and_intermediate" ]; then
    ...
    echo "       create_root_ca_only                   # Creates a new root certificate"
